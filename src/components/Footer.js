'use client';

import { Send } from "lucide-react";

export default function Footer() {
  return (
    <footer className="text-amber-50 bg-[#2D2848]">
      <div className="border-t border-white/10">
        <div className="max-w-4xl mx-auto px-6 py-10">
          
          {/* Logo and company name */}
          <a 
            href="https://thebond.company"
            target="_blank"
            rel="noopener noreferrer"
            aria-label="website"
          >
            <div className="flex flex-col items-center mb-1">
              <img 
                src="/assets/logo.png" 
                className="w-12 h-12 mb-3" 
                alt="The Bond Company Logo" 
              />
              <h3 className="text-xl font-medium tracking-normal">
                The Bond Company
              </h3>
            </div>
          </a>

          {/* Centered tagline */}
          <p className="text-center tracking-wide text-lg text-amber-50/70 mb-8 max-w-2xl mx-auto">
            Understand your emotions through simple, unforgettable analogies.
          </p>

          {/* Social links - centered */}
          <div className="flex justify-center items-center gap-5 mb-8">
            <a
              href="https://www.instagram.com/thebondcompany/"
              target="_blank"
              rel="noopener noreferrer"
              className="opacity-60 hover:opacity-100 transition-opacity duration-200"
              aria-label="Instagram"
            >
              <svg className="w-7 h-7" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
              </svg>
            </a>

            <a
              href="https://tiktok.com/@thebondcompany"
              target="_blank"
              rel="noopener noreferrer"
              className="opacity-60 hover:opacity-100 transition-opacity duration-200"
              aria-label="TikTok"
            >
              <svg className="w-7 h-7" fill="currentColor" viewBox="0 0 24 24">
                <path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-5.2 1.74 2.89 2.89 0 012.31-4.64 2.93 2.93 0 01.88.13V9.4a6.84 6.84 0 00-1-.05A6.33 6.33 0 005 20.1a6.34 6.34 0 0010.86-4.43v-7a8.16 8.16 0 004.77 1.52v-3.4a4.85 4.85 0 01-1-.1z" />
              </svg>
            </a>

            <a
              href="https://x.com/thebondco"
              target="_blank"
              rel="noopener noreferrer"
              className="opacity-60 hover:opacity-100 transition-opacity duration-200"
              aria-label="X (Twitter)"
            >
              <svg className="w-7 h-7" fill="currentColor" viewBox="0 0 24 24">
                <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
              </svg>
            </a>

            <a
              href="https://t.me/"
              target="_blank"
              rel="noopener noreferrer"
              className="opacity-60 hover:opacity-100 transition-opacity duration-200"
              aria-label="Telegram"
            >
              <Send className="w-7 h-7" />
            </a>

          </div>

          {/* Text links - centered */}
          <div className="flex flex-wrap justify-center items-center gap-x-6 gap-y-2 mb-6 text-base tracking-wide font-medium">
            <a
              href="https://thebond.company/about"
              target="_blank"
              rel="noopener noreferrer"
              className="text-amber-50/60 hover:text-amber-50 transition-colors duration-200"
            >
              About
            </a>
            <span className="text-amber-50/30">•</span>
             <a
              href="https://support.thebond.company/?section=blog"
              target="_blank"
              rel="noopener noreferrer"
              className="text-amber-50/60 hover:text-amber-50 transition-colors duration-200"
            >
              Create With Us
            </a>
            <span className="text-amber-50/30">•</span>
            <a
              href="https://support.thebond.company/?section=general"
              target="_blank"
              rel="noopener noreferrer"
              className="text-amber-50/60 hover:text-amber-50 transition-colors duration-200"
            >
              Say Hello
            </a>
           
            
          </div>

          
        </div>
      </div>
    </footer>
  );
}